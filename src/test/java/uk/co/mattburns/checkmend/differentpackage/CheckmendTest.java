...
public class CheckmendTest {
    ...
    @Test public void can_parse_error_json() {
        // @formatter:off
        ...
        assertEquals("Human readable message",
                error.getErrors().get(0).getMessage());

        assertEquals(3, error.getErrors().get(1).getId());
        assertEquals("Another human readable message",
                error.getErrors().get(1).getMessage());
        ...
        assertEquals("The required input: make was not provided.",
                error.getErrors().get(0).getMessage());
    }

    @Test public void can_generate_signature_hash() {
        ...
        assertEquals("MTpmZjYwMjM2MDI2OTU1ODA3NDNmODRmZWI4ZjI3NTBkZTg5ZTBkM2Q2",
                hash);
    }

    @Test public void can_register_person() {
    ...
    @Test public void removing_invalid_person_throws_exception() {
        Checkmend checkmend = new Checkmend(partnerid, secretKey, System.out);
        try {
            ...
            assertEquals("The Person ID supplied is invalid.",
                    e.getErrors().get(0).getMessage());
        }
    }

    @Test public void can_register_property() {
        Person bob = new Person.PersonBuilder("ref123").withFamilyname("smith")
                .withOthernames("bob").build();
    ...
    @Test public void can_encode_strings() {
        Checkmend checkmend = new Checkmend(partnerid, secretKey, System.out);

        Property property = new Property.PropertyBuilder(scfPersonid,
                Property.Category.Camera, "Canon", "0480417204")
                .withDescription("colon : and other chars !\"£$%^&*()_+‽èก")
                .build();
    ...
    @Test public void can_register_activity() {
        Person bob = new Person.PersonBuilder("ref123").withFamilyname("smith")
                .withOthernames("bob").build();
